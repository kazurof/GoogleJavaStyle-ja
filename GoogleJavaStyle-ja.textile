...
行区切り文字以外では、 *ASCII 水平スペース文字 (0x20)*  はソース内でどこに現れても良い唯一の空白文字です。つまり、
...
bq. *TIP:* Unicodeエスケープの場合時たま実際にUnicode文字が使われている時でも、説明のコメントがあるとわかりやすいです。
...
bq. *TIP:* 単に何かのプログラムが非ASCII文字を正しく処理しないからといってコードを読みにくくしないで下さい。もしそのような事が起こる場合はそのプログラムがおかしいのであってそちらが修正されるべきです。
