...
本文書はGoogleのJavaコーディング規約であるところの "Google Java Style Guide":https://google.github.io/styleguide/javaguide.html の非公式和訳です。"2016/06/30":https://github.com/google/styleguide/blob/c938dbaa4dadecb2ca02791d7c4ad3ff7d6fe78a/javaguide.html の版を使っています。技術的に正確である事を意図して訳してありますが、どこかで間違えているかもしれません。本家は随時更新される様子ですが、こちらが追いつかなくなるかもしれません。生暖かく見守ってやってください。誤訳、誤植の指摘を歓迎いたします。
...
ソースファイル名はそのファイルに(正確に一つ)入っているトップレベルクラスの大文字小文字を区別した名前と加えて @.java@ という拡張子が付いていること。
...
文字列リテラルやコメント以外でのUnicodeエスケープは強く推奨されないが、
この選択は *ソースコードを読むことや理解することが簡単になる* かどうかのみに依存する。
...
クラスに複数のコンストラクタや同じ名前を持つメソッドがある場合は別のメンバーを（privateなものであっても）入れることなく連続して並べる。
...
* 開始中括弧の後に改行を入れる。
* 終了中括弧の前に改行を入れる。
* もし終了中括弧が文やメソッドの本体を終えるならばその時のみ、その中括弧の後に改行を入れる。例えば終了中括弧の後に @else@ や、カンマが続く場合は改行をしない。
...
*用語の注記：* ある意味では規約に合致しているコードを複数行に分ける場合、この活動を行の折り返しと呼ぶ。
