...
fn1. 訳注：JavaScript Native Interface 。Google Web Tookit においてアプリからJavaScriptコードを呼ぶことができる。
...
識別子はASCII文字、数字、後述する2つの場合(訳注:5.2.4の "定数名":#s5-2-4-constant-names の場合と、5.2.3の "メソッド名":#s5-2-3-method-names でのUnitのテストメソッド名の場合)はアンダースコアのみを使う。それゆえ、有効な識別子名は正規表現 @\w+@ にマッチする。
...
bq. *Tip:* これをやってはならない。絶対的に必要ならば、まず "Effective Java":http://books.google.co.jp/books?id=ka2VUBqHiWkC のItem7「ファイナライザを避ける」を必ず熟読しなくてはならない。そしてこれをやってはならない。
...
h4(#s7-1-3-javadoc-at-clauses). 7.1.3 Javadoc タグ [3]
...
fn3. 訳注：Javadoc での @param といった節のこと。原文では"at-clauses"であるが、GoogleJavaStyleでしか使っていない用語なのでJavadocの元文書に基づきJavadoc タグと翻訳した。参照 "Javadoc タグ":http://docs.oracle.com/javase/jp/8/docs/technotes/tools/windows/javadoc.html#CHDBEFIF
