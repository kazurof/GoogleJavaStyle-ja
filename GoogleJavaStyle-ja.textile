...
新しいブロックあるいはブロック構造物が開始した時インデントは空白2個づつ増える。ブロックが終了したら、インデントは1個まえのレベルに戻る。
インデントレベルはブロックを通じてコードとコメントに適用される。4.1.2節の例を参照のこと。（ 4.1.2 空でないブロックではK&Rスタイルに従う。）
...
"4.6.3節の水平位置揃え"はあるトークンを前の行に揃えるため色々な数の空白を使うとげんなりするやり方を防止します。
